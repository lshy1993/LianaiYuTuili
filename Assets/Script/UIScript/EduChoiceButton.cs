...
namespace Assets.Script.UIScript
{
    public class EduChoiceButton :MonoBehaviour
    {
        ...
        void OnClick()
        {
            ...
            Debug.Log("User状态：" + User.GetInstance().ToString());
        }
        ...
        private void RunChoice()
        {
            foreach(var item in choice.data)
            {
                ...
                User.GetInstance().SetClassDelta(item.Key, UnityEngine.Random.Range(min, max));
                User.GetInstance().SetStatusDelta(item.Key, UnityEngine.Random.Range(min, max));
            }
        }
    }
}
