...
public class FinUIManager : MonoBehaviour
{
    ...
    {
        picPanel.GetComponent<UI2DSprite>().sprite2D = null;
    ...
    private IEnumerator ShowPic()
    {
        ...
        {
            //随机显示图片
            picPanel.GetComponent<UI2DSprite>().sprite2D = spAll[UnityEngine.Random.Range(0, spAll.Count())];
            ...
            while (t < 1)
            {
                ...
                picPanel.GetComponent<UIWidget>().alpha = t;
                yield return null;
            }
            yield return new WaitForSeconds(5f);
            while (t > 0)
        ...
        //2
        picPanel.GetComponent<UI2DSprite>().sprite2D = Resources.Load<Sprite>("Background/school_day");
        while (t < 1)
        {
            t = Mathf.MoveTowards(t, 1, 1 / 0.5f * Time.deltaTime);
            picPanel.GetComponent<UIWidget>().alpha = t;
