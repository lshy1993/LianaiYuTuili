...
public class DialogBoxUIManager : MonoBehaviour
{
    ...
    private UI2DSprite backSprite, avatarSprite;
    // 文字履历表
    ...
    void Awake()
    {
        ...
        backSprite = mainContainer.transform.Find("DialogBack_Sprite").GetComponent<UI2DSprite>();
    ...
    public void InitDialogBox(string filename, int x, int y, int w, int h)
    {
        mainContainer.transform.localPosition = new Vector2(x, y);
        UIWidget box = mainContainer.GetComponent<UIWidget>();
        box.height = h;
        box.width = w;
        ...
        {
            Debug.Log(filename);
            backSprite.sprite2D = Resources.Load<Sprite>("UI/" + filename);
            backSprite.width = w;
            backSprite.height = h;
        }
    }

    public void InitDialogLabel(int left, int top, int right, int bottom)
    {
        // 图片范围
        UIWidget box = mainContainer.GetComponent<UIWidget>();
        int w = box.width;
        int h = box.height;
        // 边间距
        dialogLabel.transform.localPosition = new Vector2(left-w/2, h/2-top);
        dialogLabel.width = w - left - right;
        dialogLabel.height = h - top - bottom;
    }
    ...
    private void SetAvatar(string filename)
    {
        ...
        {
            avatarSprite.sprite2D = Resources.Load<Sprite>("Character/" + filename);
